...
static GLFWbool modeIsGood(CGDisplayModeRef mode)
{
    ...
    format = CGDisplayModeCopyPixelEncoding(mode);
...
GLFWbool _glfwSetVideoModeNS(_GLFWmonitor* monitor, const GLFWvidmode* desired)
{
    ...
    for (i = 0;  i < count;  i++)
    {
        dm = (CGDisplayModeRef) CFArrayGetValueAtIndex(modes, i);
        if (!modeIsGood(dm))
            continue;

        mode = vidmodeFromCGDisplayMode(dm, link);
...
_GLFWmonitor** _glfwPlatformGetMonitors(int* count)
{
    ...
    *count = 0;
    ...
    for (i = 0;  i < displayCount;  i++)
    {
        ...
        size = CGDisplayScreenSize(displays[i]);
        name = getDisplayName(displays[i]);
